...
export const DateRangeSelector: FC<DateRangeSelectorProps> = ({
	...
	const startDatePlaceholder = lastIndexedAt
		? `Från ${formatRelativeDate(lastIndexedAt)}`
		: "Standard (1 år)";
	...
	return (
		<div className="rounded-xl bg-slate-400/5 dark:bg-white/5 p-3 sm:p-6">
			<h3 className="font-medium text-sm sm:text-base mb-4">Välj datumintervall</h3>
			<p className="text-xs sm:text-sm text-muted-foreground mb-6">
				{allowFutureDates
					? "Välj datumintervall för att synkronisera dina data. Du kan välja framtida datum för att indexera kommande händelser."
					: "Välj hur långt tillbaka du vill synkronisera dina data. Du kan alltid indexera om senare med andra datum."}
			</p>
				...
				<div className="space-y-2">
					...
					<Popover>
						<PopoverTrigger asChild>
							...
							>
								<CalendarIcon className="mr-2 h-4 w-4" />
								{endDate ? format(endDate, "PPP") : "Standard (Idag)"}
							</Button>
						</PopoverTrigger>
						<PopoverContent className="w-auto p-0 z-[100]" align="start">
							<Calendar
