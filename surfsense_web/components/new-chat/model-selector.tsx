...
export function ModelSelector({ onEdit, onAddNew, className }: ModelSelectorProps) {
	...
	const handleSelectConfig = useCallback(
		async (config: NewLLMConfigPublic | GlobalNewLLMConfig) => {
			...
			if (!searchSpaceId) {
				toast.error("Inget sökutrymme valt");
				return;
			}
			...
			try {
				...
				toast.success(`Bytte till ${config.name}`);
				setOpen(false);
			} catch (error) {
				console.error("Failed to switch model:", error);
				toast.error("Det gick inte att byta modell");
			}
		},
				...
				>
					{isLoading ? (
						<>
							...
							<span className="text-muted-foreground hidden md:inline">Läser in</span>
						</>
					) : currentConfig ? (
						<>
						...
						<>
							...
							<span className="text-muted-foreground hidden md:inline">Välj modell</span>
						</>
					)}
					<ChevronDown
						className={cn(
					...
					<CommandList className="max-h-[300px] md:max-h-[400px] overflow-y-auto">
						<CommandEmpty className="py-8 text-center">
							<div className="flex flex-col items-center gap-2">
								...
								<p className="text-sm text-muted-foreground">Inga modeller hittades</p>
								<p className="text-xs text-muted-foreground/60">Prova en annan sökterm</p>
							</div>
						</CommandEmpty>
													...
													<div className="min-w-0 flex-1">
														...
														<div className="flex items-center gap-1.5 mt-0.5">
															<span className="text-xs text-muted-foreground truncate">
																{isAutoMode ? "Automatisk lastbalansering" : config.model_name}
															</span>
						...
						<div className="p-2 bg-muted/20">
							...
							>
								...
								<span className="text-sm font-medium">Lägg till ny konfiguration</span>
							</Button>
						</div>
					</CommandList>
				</Command>
