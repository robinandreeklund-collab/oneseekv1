    ...
    *,
    ...
    async def smart_critic_node(
        ...
        **kwargs,
        ...
        if not has_success and avg_confidence < 0.5:
            if replan_count < max_replan_attempts:
                logger.info(
            ...
            # Budget exhausted with no success and no usable response.
            # Generate an honest disclaimer so the user isn't left with
            # an empty screen or hallucinated data.
            if not final_response:
                disclaimer = (
                    "Tyvärr kunde jag inte hämta den efterfrågade datan. "
                    "Verktygen returnerade ofullständiga eller felaktiga resultat "
                    "efter flera försök. Försök gärna igen eller omformulera frågan."
                )
                logger.warning(
                    "smart_critic: budget exhausted with no response, "
                    "generating disclaimer (avg_confidence=%.2f, replan_count=%d)",
                    avg_confidence,
                    replan_count,
                )
                await _record_feedback(False)
                return {
                    ...
                    "final_agent_response": disclaimer,
                    "final_agent_name": "supervisor",
                    "orchestration_phase": "finalize",
                    "critic_history": critic_history + [
                        {
                            "decision": "ok",
                            "reason": "budget_exhausted_disclaimer",
                            "step": total_steps,
                        }
                    ],
                }
